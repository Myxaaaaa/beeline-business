    ...
    <div className={style.resultsContainer}>
      {query.length < 3 && (
        <div className={style.searchingContainer}>
          <h2 className={style.resultText}>Введите не менее 3 букв</h2>
      ...
      {!isLoading && query.length >= 3 && error && (
        <div className={style.searchResults}>
          ...
          {error === 404 ? (
            ...
            <div className={style.errorResult}>
              <p className={style.resultText}>УПССС... Внутренняя ошибка сервера</p>
              <div className={style.shadow}>
                <img className={style.noDataImg} src={serverError} alt="Server error" />
              </div>
            </div>
          )}
        </div>
            ...
            <ul className={style.resultsList}>
              {results.map((result, index) => (
                <li key={`${result.id}-${index}`} className={style.resultItem}>
                  <a href={result.url} className={style.resultLink} rel="noopener noreferrer">
